...
class _VoiceAssistantState extends State<VoiceAssistant> {
  ...
  String lastWords = 'Say the keyword page that you want to visit.(ie:Add Vital or Supplement Checklist)';
  String lastError = '';
  String lastStatus = '';
              ...
              child: Column(
                  children: [
                    Expanded(
                        child:
                        speech.isListening?
                        LoadingAnimationWidget.beat(color: Colors.lightBlueAccent, size: 100):const SizedBox()),
                    Expanded(
  ...
  Future _speak(spokenText) async {
    ...
    switch(page) {
      case "main dashboard":{
        ...
        else if(spokenText=='jarvis'||spokenText.contains('jarvis')||spokenText.contains('ok jarvis')||spokenText.contains('hey jarvis')||spokenText.contains('hello jarvis')||spokenText.contains('hello')||spokenText.contains('shfc')||spokenText.contains('hfc')||spokenText.contains('heart app')){
        ...
        else if(spokenText=='add vitals'||spokenText.contains('vitals')||spokenText.contains('vital')||spokenText.contains('advertise')||spokenText.contains('adwitter')){
          Get.back();
          MyNavigator.push(context, const AddVitalView());

          flutterTts.speak("This is your add vitals page");
        }
        else if(spokenText=='prescription checklist'||spokenText.contains('prescription')||spokenText.contains('prescription checklist')||spokenText.contains('prescription')){
          Get.back();
          MyNavigator.push(context, const PillsReminderView());

          flutterTts.speak("This is your prescription page");
        }
        else if(spokenText=='symptom'||spokenText.contains('symptoms')||spokenText.contains('symptom tracker')||spokenText.contains('symptoms tracker')||spokenText.contains('track symptom')){
          Get.back();
          MyNavigator.push(context, const SymptomTracker());
          flutterTts.speak("This is your Symptom page");
        }

        else if(spokenText=='food'||spokenText.contains('food intake')||spokenText.contains('diet')||spokenText.contains('food')||spokenText.contains('diet tracking')){
          Get.back();
          MyNavigator.pushReplacement(context, const FoodIntakeView());
          flutterTts.speak("This is your Diet Checklist Page");
        }
        else if(spokenText.contains('intake')||spokenText.contains('output')||spokenText.contains('glass')||spokenText.contains('fluid')){
          Get.back();
          MyNavigator.push(context, const SliderVerticalWidget());
          flutterTts.speak("This is your Fluid Intake Page");
        } else if(spokenText=='supplement'||spokenText.contains('intake')||spokenText.contains('supplement')){
          Get.back();
          MyNavigator.push(context, const SupplementIntakeView());
          flutterTts.speak("This is your supplement CheckList page");

        }
        else if(spokenText=='upload report'||spokenText.contains('upload report')||spokenText.contains('upload')||spokenText.contains('report')||spokenText.contains('file')||spokenText.contains('photo')){
          Get.back();
        ...
        else if(spokenText=='tell me about the app'||spokenText.contains('s h f c')||spokenText.contains('shfc')||spokenText.contains('app')){
          flutterTts.speak('hi there I am here to help you and make your work easier, this is  SMART HEART FAILURE COMPANION  application');
        }
        else if(spokenText=='life style'||spokenText.contains('lifesyle')||spokenText.contains('lifeStyle intervention')||spokenText.contains('lifeStyles')||spokenText.contains('lifeStyle lnterventions')||spokenText.contains('lifeStyles lnterventions')){
          Get.back();
        ...
        else if(spokenText=='urine'||spokenText.contains('urine Output')||spokenText.contains('urine output page')){
          Get.back();
          MyNavigator.push(context, const UrinOutputView());
          flutterTts.speak("ok this is your Urine Output Page");
        }
        else if(spokenText=='exercise tracker'||spokenText.contains('exercise Tracking')||spokenText.contains('exercise tracking Page')||spokenText.contains('exercise')||spokenText.contains('exercise page')){
          Get.back();
          MyNavigator.push(context, const ExerciseTrackingView());
          flutterTts.speak("ok this is your Exercise Tracker Page");
        }

        else if(spokenText=='call'||spokenText.contains('emergency Call')){
          String morningTime='';
    ...
    //
      ...
      case "add vitals": {
        AddVitalViewModal addvitalVM =
        Provider.of<AddVitalViewModal>(context, listen: false);
        if(spokenText.contains('systolic')||spokenText.contains('solic')||spokenText.contains('scrollet')||spokenText.contains('install it')||spokenText.contains('trolley')||spokenText.contains('historic')||spokenText.contains('strongest')||spokenText.contains('tallest')||spokenText.contains('installing')){
          int number =  getNumber(spokenText);
          addvitalVM.systollicC.text = number.toString();
          flutterTts.speak('added systolic');

        }
       else if(spokenText.contains('diastolic')||spokenText.contains('that\'s qualification')||spokenText.contains('dastolic')){
          int number =  getNumber(spokenText);
          addvitalVM.diatollicC.text = number.toString();
          flutterTts.speak('added  diastolic');
        }
       else if(spokenText.contains('pulse')||spokenText.contains('pulse rate')){
          int number =  getNumber(spokenText);
          addvitalVM.pulserateC.text = number.toString();

          flutterTts.speak('added pulse');
        }
       else if(spokenText.contains('heart')||spokenText.contains('heartbeat')||spokenText.contains('hard')){
          int number =  getNumber(spokenText);
          addvitalVM.heartrateC.text = number.toString();

          flutterTts.speak('added heart rate');
        }
       else if(spokenText.contains('spo2')||spokenText.contains('s p o 2')||spokenText.contains('sp auto')||spokenText.contains('sq2')||spokenText.contains('po2')||spokenText.contains('o2')||spokenText.contains('you tube')){
          int number =  getNumber(spokenText);
          addvitalVM.spo2C.text = number.toString();
          flutterTts.speak('added  s p o 2');
        }  else if(spokenText.contains('temperature')||spokenText.contains('fever')){
          int number =  getNumber(spokenText);
          addvitalVM.temperatureC.text = number.toString();
          flutterTts.speak('added  temperature');
        }
       else if(spokenText.contains('respiratory rate')||spokenText.contains('respiratory')||spokenText.contains('respiration')||spokenText.contains('battery rate')){
          int number =  getNumber(spokenText);
          addvitalVM.respiratoryC.text = number.toString();
          flutterTts.speak('added  respiratory rate');
        }
       else if(spokenText.contains('rbs')||spokenText.contains('r b s')){
          int number =  getNumber(spokenText);
          addvitalVM.rbsC.text = number.toString();

          flutterTts.speak('added  R B S');
        }
       else if(spokenText.contains('weight')||spokenText.contains('wait')||spokenText.contains('kilo')||spokenText.contains(' kg')||spokenText.contains('gram')){
          int number =  getNumber(spokenText);
          addvitalVM.weightC.text = number.toString();
          flutterTts.speak('added weight');
        }
       else if(spokenText.contains('save')||spokenText.contains('upload')){

          if (addvitalVM.systollicC.text != '' ||
              addvitalVM.diatollicC.text != ''||addvitalVM.pulserateC.text != '' ||
  ...
  bool weAreListening = true;
    ...
    Future<void> postData(text) async {
      ...
      ApplicationLocalizations localization =
      Provider.of<ApplicationLocalizations>(context, listen: false);
      MedicineViewCheckListDataMOdel medicineVm = Provider.of<MedicineViewCheckListDataMOdel>(context, listen: false);
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if(
          addvitalVM.allData['vmValueBPSys'].toString()=='0'&&
          ...
          if( addvitalVM.allData['fluidValue'].toString()!='{}'){
            ...
            Get.back();
            await Get.to(()=> SliderVerticalWidget(throughVoice: true,));
          ...
          if(addvitalVM.allData['symptomsList'].toString()!='[]'){
            ...
            symptomtrackerVM.symptomsVoiceList=addvitalVM.allData['symptomsList'];
            Get.back();
            ...
            // }
            Get.to(()=> SymptomTracker(throughVoice: true,));
...
//   }
  ...
  getNumber(text){
    int number = int.parse(RegExp(r'\d+').firstMatch(text.toString().replaceAll('o2', ''))!.group(0)!);
    return number??0;
  }
        ...
        children: <Widget>[
          ...
          Positioned.fill(
            ...
            child: Padding(
              ...
              child: Visibility(
                visible: lastWords!='',
                replacement: const Text('Say the keyword page that you want to visit.\n'
                    'ie: Add Vital or Supplement Checklist', textAlign: TextAlign.center,style: TextStyle(
